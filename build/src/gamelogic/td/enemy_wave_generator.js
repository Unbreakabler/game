import { EnemyType, ENEMY_MODIFIERS, TIER_MODIFIER_WEIGHTS } from './stats_enemy_modifiers.js';
import ENEMY_BASE_STATS, { calculateEnemyDifficulty } from './stats_base_enemies.js';
 ...
 * @returns items[i]
 */
function weightedRandom(items, pop) {
    let i;
    let weights = [];
    for (i = 0; i < items.length; i++) {
        if (typeof items[i].weight !== undefined) {
...
const calculateMaxMobDifficulty = (wave_type, max_difficulty) => {
    let divisor = 10;
    if (wave_type === 'magic')
        divisor = 4;
    if (wave_type === 'rare')
 ...
 */
const chooseEnemyType = (max_difficulty) => {
    /*
...
const generateEnemyList = (mob_with_mods_difficulty, modifiers, max_difficulty) => {
